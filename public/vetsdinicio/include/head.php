<?php
if(!isset($titulo)) {
	$titulo = 'SDI';
}
?>
<head>
	<meta charset="utf-8">
	<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">
	<title>VET SDI</title>
	<link rel="stylesheet" type="text/css" href="assets/css/bootstrap.min.css">
	<link rel="stylesheet" type="text/css" href="assets/css/owl.carousel.min.css">
	<!-- Estilos -->
  	<link href="assets/css/main.css" rel="stylesheet" type="text/css">
  	<!-- Banners y Sliders -->
  	<link href="assets/css/banners-sliders.css" rel="stylesheet" type="text/css">
  	<link href="assets/css/aos.css" rel="stylesheet" type="text/css">
  	<script src="https://kit.fontawesome.com/7066bc9f2e.js" crossorigin="anonymous"></script>

  	
  	<script src="https://kit.fontawesome.com/7066bc9f2e.js" crossorigin="anonymous"></script>
	<script src="assets/js/datatables.min.js"></script>
  	

	<!-- Google Fonts -->
	<link rel="preconnect" href="https://fonts.googleapis.com">
	<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
	<link href="https://fonts.googleapis.com/css2?family=Inter:ital,opsz,wght@0,14..32,100..900;1,14..32,100..900&display=swap" rel="stylesheet">

	<script src="https://cdnjs.cloudflare.com/ajax/libs/moment.js/2.29.4/moment.min.js"></script>

	<?php if (!empty($extra_css) && is_array($extra_css)): ?>
		<?php foreach ($extra_css as $extra_css_file): ?>
			<link rel="stylesheet" type="text/css" href="<?php echo $extra_css_file; ?>">
		<?php endforeach; ?>
	<?php endif; ?>

	<?php if (!empty($extra_head)): ?>
		<?php echo $extra_head; ?>
	<?php endif; ?>
</head>
